...
<code>git clone https://github.com/mstrauss/puppet-editfile.git modules/editfile</code>

If you use submodules:

<code>git submodule add https://github.com/mstrauss/puppet-editfile.git modules/editfile</code>
...
+editfile+ is a custom type (+lib/puppet/type/editfile.rb+) with a single custom provider (+lib/puppet/provider/editfile/simple.rb+). The _simple_ provider can replace lines matching a regular expression with given content.
...
The example from above without the <tt>editfile::config</tt> wrapper:
  ...
  editfile { 'sshd: disallow root':   # use a unique, descriptive name
    provider => simple,               # that's the default provider
    path     => '/etc/ssh/sshd_config',
